  ...
  return (
    <div style={loginBannerStyle} className="min-h-screen bg-purple-700">
      <div className="container mx-auto flex items-center justify-between min-h-screen">
        <div></div>
        <div className="bg-purple-200  w-full max-w-xl py-16 px-8">
       {!isLoading && <div>
          <h2 className="text-purple-800 mb-6 text-3xl font-bold ">
            Login please
          </h2>
          <form onSubmit={submitLoginForm} className="mb-4">
            <input
              onBlur={onBlurInput}
          ...
          <Link to="/register" className="font-medium text-red-700 text-lg">
            Are you new user?
          </Link>
         </div>}
          {isLoading && (
